...
interface AdminFormProps {
  ...
  onSubmit: (data: AdminFormPayload) => Promise<void>;
  onCancel: () => void;
  submitting: boolean;
}
...
const validateField = (name: keyof AdminFormPayload, value: string): string => {
  switch (name) {
    case "first_name":
...
const AdminForm: React.FC<AdminFormProps> = ({
  ...
  const [formData, setFormData] = useState<AdminFormPayload>(EMPTY);
  const [errors, setErrors]     = useState<Partial<Record<keyof AdminFormPayload, string>>>({});
  const [touched, setTouched]   = useState<Partial<Record<keyof AdminFormPayload, boolean>>>({});

  const validate = (name: keyof AdminFormPayload, value: string) =>
    validateField(name, value);

  const handleChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>
  ) => {
    const name  = e.target.name as keyof AdminFormPayload;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const fields: (keyof AdminFormPayload)[] = [
      "first_name", "last_name", "email", "phone", "region",
    ];
  ...
  const showError = (name: keyof AdminFormPayload) =>
        ...
        <div>
          ...
          <select
            ...
            className={`w-full px-3 py-2 border ${
              showError("region") ? "border-red-500 dark:border-red-500" : "border-gray-300 dark:border-gray-600"
