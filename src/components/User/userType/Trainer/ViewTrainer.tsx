...
import TrainerForm, { TrainerFormPayload } from "./TrainerForm";
...
const ViewTrainer: React.FC = () => {
  ...
  const handleStatusChange = async (userId: number, newStatus: boolean): Promise<void> => {
  ...
  const confirmSendEmail = async () => {
    ...
    try {
      ...
      if (modalConfig.type === "reset") {
        await requestPasswordApi({ email: emailToSend });
        toast.success(`Password reset email sent successfully to ${emailToSend}`);
      } else {
        await resendInvitationApi({ email: emailToSend });
  ...
  const handleSubmit = async (data: TrainerFormPayload): Promise<void> => {
    setSubmitting(true);
    ...
    try {
      ...
      if (isAdmin) {
        ...
        if (!data.mnpUser) { toast.error("MNP User is required"); return; }
        managerId = parseInt(data.mnpUser);
        if (isNaN(managerId)) { toast.error("Invalid MNP User selected"); return; }
      }
    ...
    () => [
      {
        ...
        Cell: ({ row }: { row: MRT_Row<User> }) => {
          ...
          const rowActions: RowAction[] = [
            { label: "Reset Password",    onClick: () => handleResetPassword(userEmail) },
            ...(!inviteAccepted && userEmail
              ? [{ label: "Resend Invitation", onClick: () => handleResendInvitation(userEmail) }]
              : []),
          ];
      ...
      },
      {
        ...
        Cell: ({ cell }: { cell: MRT_Cell<User> }) => (cell.getValue() as string | null) || "-",
          ...
          if (isEditing) {
            return (
              <div className="flex items-center gap-2">
                <select value={editingStatus?.isActive ? "active" : "inactive"}
                  onChange={(e) => setEditingStatus({ userId, isActive: e.target.value === "active" })}
                  className="px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-800 dark:text-white" autoFocus>
                  <option value="active">Active</option>
                  <option value="inactive">Inactive</option>
                </select>
                <button onClick={() => editingStatus && handleStatusChange(userId, editingStatus.isActive || false)}
  ...
  const toolbarActions: ToolbarAction[] = [
    ...(canAddUsers ? [{ label: "Add Trainer", onClick: handleAddUser }] : []),
  ...
  return (
    <div className="p-3">
      ...
      <div className="mb-6">
        <h1 className="font-semibold text-gray-800 text-title-sm dark:text-white/90 sm:text-title-md">Trainer Management</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400">
          {isSuperAdmin ? "Manage and create all users Trainer" : "View all users in the system"}
        </p>
        {!loading && (
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-theme-sm">
        <CommonTable
          ...
          dropdownLabel="Trainer Actions" columnFilters={columnFilters}
      ...
      {canAddUsers && (
        <RightSideModal isOpen={isAddModalOpen} onClose={handleCloseModal} showCloseButton width=" ">
          <div className="p-3">
            <h2 className="text-xl font-semibold text-gray-800 dark:text-white/90 mb-4">Add New Trainer</h2>
            <TrainerForm
              key={String(isAddModalOpen)} isAdmin={isAdmin} roleList={roleList}
              onSubmit={handleSubmit} onCancel={handleCloseModal} submitting={submitting}
            />
          </div>
        </RightSideModal>
      )}
      ...
      <ConfirmModal
        ...
        onConfirm={confirmSendEmail} email={emailToSend} loading={sendingEmail}
