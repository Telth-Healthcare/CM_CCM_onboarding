...
interface FinancierFormProps {
  ...
  onSubmit: (data: FinancierFormPayload) => Promise<void>;
  onCancel: () => void;
  submitting: boolean;
}

export interface FinancierFormPayload {
  first_name: string;
  last_name: string;
  email: string;
  phone: string;
...
const validateField = (
  name: keyof FinancierFormPayload,
  value: string,
  isAdmin: boolean
): string => {
  switch (name) {
...
const EMPTY: FinancierFormPayload = {
  first_name: "",
  last_name: "",
  email: "",
  phone: "",
...
const FinancierForm: React.FC<FinancierFormProps> = ({
  isAdmin,
  roleList,
  onSubmit,
  onCancel,
  ...
  const [formData, setFormData] = useState<FinancierFormPayload>(EMPTY);
  const [errors, setErrors]     = useState<Partial<Record<keyof FinancierFormPayload, string>>>({});
  const [touched, setTouched]   = useState<Partial<Record<keyof FinancierFormPayload, boolean>>>({});

  const validate = (name: keyof FinancierFormPayload, value: string) =>
    validateField(name, value, isAdmin);

  const handleChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>
  ) => {
    const name  = e.target.name as keyof FinancierFormPayload;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const fields: (keyof FinancierFormPayload)[] = [
  ...
  const showError = (name: keyof FinancierFormPayload) =>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) => handleNameChange(e, "first_name")} onBlur={handleBlur}
            placeholder="Enter first name" error={!!showError("first_name")}
          />
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) => handleNameChange(e, "last_name")} onBlur={handleBlur}
            placeholder="Enter last name" error={!!showError("last_name")}
          />
          {showError("last_name") && (
        ...
        <div>
          ...
          <Input
            ...
            placeholder="Enter email address" error={!!showError("email")}
          />
          {showError("email") && (
        ...
        {!isAdmin && (
          <div>
            ...
            <select
              name="mnpUser" value={formData.mnpUser}
              onChange={handleChange} onBlur={handleBlur}
              className={`w-full px-3 py-2 border ${
                showError("mnpUser") ? "border-red-500 dark:border-red-500" : "border-gray-300 dark:border-gray-600"
              } rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-800 dark:text-white`}
            >
              <option value="" disabled>Select a MNP User</option>
              {roleList.map((a) => (
                <option key={a.value} value={a.value}>{a.label}</option>
              ))}
            </select>
            {showError("mnpUser") && (
              <p className="mt-1 text-sm text-red-600 dark:text-red-400">{errors.mnpUser}</p>
            )}
          </div>
        )}
      </div>
