...
// ─── Mock certificates (tied to modules) ─────────────────────────────────────

const CERTIFICATES: Certificate[] = [
  {
    id: "c1",
    moduleTitle: "Community Care Foundations",
  ...
  {
    id: "c3",
    moduleTitle: "Digital Tools & Platforms",
    issuedDate: "",
    status: "locked",
    progress: 0,
  },
  ...
  const [notes, setNotes] = useState<Note[]>([
    {
      ...
      content:
        "Remember: community-first approach. Stakeholder mapping is critical before any deployment. Always document interactions.",
    ...
    },
    {
      ...
      content:
        "Safety kit, ID card, reporting forms, emergency contacts list, GPS device charged.",
        ...
        <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-5 space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {NOTE_COLORS.map((c) => (
              <button
                ...
                className={`w-6 h-6 rounded-full border-2 transition ${c.split(" ")[0]} ${
                  formColor === c ? "border-gray-500 scale-110" : "border-transparent"
                }`}
              />
            ))}
          </div>
          <div className="flex gap-2 justify-end">
            <button
              ...
              className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-white/[0.03] transition"
            >
              Cancel
            </button>
            <button
      ...
      {notes.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {notes.map((note) => (
            <div
              key={note.id}
              className={`rounded-2xl border p-4 flex flex-col gap-2 ${note.color}`}
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex gap-1 flex-shrink-0">
                  <button
                    ...
                    className="p-1 rounded hover:bg-black/10 text-gray-500 dark:text-gray-400 transition"
                  >
                  ...
                  <button
                    ...
                    className="p-1 rounded hover:bg-red-100 text-gray-500 hover:text-red-500 transition"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400 line-clamp-4 flex-1 whitespace-pre-wrap">
                {note.content}
              </p>
...
function SubmissionsTab() {
  ...
  const submitDocument = () => {
    ...
    const newSub: Submission = {
      ...
      fileType: ["PDF"].includes(ext) ? "PDF" : ["JPG", "JPEG", "PNG"].includes(ext) ? "Image" : ext,
      status: "submitted",
    ...
    setUploadName("");
    setUploadCategory(SUBMISSION_CATEGORIES[0]);
  };

  const FILE_ICON = (type: string) => {
    if (type === "PDF")
      return <div className="w-9 h-9 rounded-lg bg-red-100 dark:bg-red-900/20 flex items-center justify-center text-red-500 text-[10px] font-bold">PDF</div>;
    if (type === "Image")
      return <div className="w-9 h-9 rounded-lg bg-blue-100 dark:bg-blue-900/20 flex items-center justify-center text-blue-500 text-[10px] font-bold">IMG</div>;
    ...
    <div className="space-y-5">
      ...
      {showUpload && (
        <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-5 space-y-4">
          ...
          <div className="flex gap-2 justify-end">
            <button
              onClick={() => { setShowUpload(false); setSelectedFile(null); setUploadName(""); }}
              className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-white/[0.03] transition"
            >
              Cancel
            </button>
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-white bg-brand-500 rounded-lg hover:bg-brand-600 disabled:opacity-50 disabled:cursor-not-allowed transition"
            >
              Submit to Backend
            </button>
          </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  {sub.fileName} · {sub.fileSize} · {formatDate(sub.uploadedAt)}
                </p>
              </div>
              <span
                className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${STATUS_STYLES[sub.status]}`}
              >
...
const TABS: { key: Tab; label: string; icon: React.ReactNode; desc: string }[] = [
  ...
  },
  {
    key: "certificates",
    label: "Certificates",
    desc: "Module completion certificates",
    icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" />
      </svg>
