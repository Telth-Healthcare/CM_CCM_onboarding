...
const CCMUserDropdown: React.FC = () => {
  const [isOpen, setIsOpen] = useState(false);
  const navigate = useNavigate();

  // ccm_user stored as: { user: {id:7, first_name, ...}, profile_id: 3, ... }
  const ccmUser   = JSON.parse(localStorage.getItem("ccm_user") || "null");
  const innerUser = ccmUser?.user ?? ccmUser;   // get the nested user object

  if (!innerUser) return null;

  const firstName = innerUser.first_name || "User";
  const lastName  = innerUser.last_name  || "";
  const email     = innerUser.email      || "";
  ...
  return (
    <div className="relative">
      ...
      >
        {/* Avatar with initials instead of placeholder image */}
      ...
      >
        ...
        <div className="flex items-center gap-3 pb-3 border-b border-gray-100 dark:border-gray-800">
          ...
          <div className="min-w-0">
            <span className="block font-medium text-gray-700 text-theme-sm dark:text-gray-400 capitalize truncate">
              {firstName} {lastName}
            </span>
...
export default CCMUserDropdown;